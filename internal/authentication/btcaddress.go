...
func getP2PKHAddress(publicKey *btcec.PublicKey) string {
...
func getP2SHAddress(publicKey *btcec.PublicKey) string {
...
func getNativeSegWitAddress(publicKey *btcec.PublicKey) string {
	witnessProg := btcutil.Hash160(publicKey.SerializeCompressed())
	addressWitnessPubKeyHash, err := btcutil.NewAddressWitnessPubKeyHash(witnessProg, &chaincfg.MainNetParams)
	if err != nil {
		return ""
...
func getTaprootAddress(publicKey *btcec.PublicKey) string {
	tapKey := computeTaprootOutputKey(publicKey, []byte{})
